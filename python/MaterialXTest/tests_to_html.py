...
def main(args=None):
    ...
    parser.add_argument(
        "-i2", "--inputdir2", dest="inputdir2", action="store", help="Second input directory", default="."
    )
    parser.add_argument(
        "-i3", "--inputdir3", dest="inputdir3", action="store", help="Third input directory", default="."
    )
    parser.add_argument(
        "-o", "--outputfile", dest="outputfile", action="store", help="Output file name", default="tests.html"
    )
    parser.add_argument(
        "-d", "--diff", dest="CREATE_DIFF", action="store_true", help="Perform image diff", default=False
    )
    parser.add_argument(
        "-t", "--timestamp", dest="ENABLE_TIMESTAMPS", action="store_true", help="Write image timestamps", default=False
    )
    parser.add_argument(
        "-w", "--imagewidth", type=int, dest="imagewidth", action="store", help="Set image display width", default=256
    )
    parser.add_argument(
        "-ht",
        "--imageheight",
        ...
        dest="imageheight",
        action="store",
        help="Set image display height",
        default=256,
    )
    parser.add_argument(
        "-cp", "--cellpadding", type=int, dest="cellpadding", action="store", help="Set table cell padding", default=0
    )
    parser.add_argument(
        "-tb",
        "--tableborder",
        ...
        dest="tableborder",
        action="store",
        help="Table border width. 0 means no border",
        default=3,
    )
    parser.add_argument(
        "-l1",
        ...
        dest="lang1",
        action="store",
        help="First target language for comparison. Default is glsl",
        default="glsl",
    )
    parser.add_argument(
        "-l2",
        ...
        dest="lang2",
        action="store",
        help="Second target language for comparison. Default is osl",
        default="osl",
    )
    parser.add_argument(
        "-l3",
        ...
        dest="lang3",
        action="store",
        help="Third target language for comparison. Default is empty",
        default="",
    )
    ...
    if langFiles1:
        curPath = ""
        for file1, file2, file3, path1, path2, path3 in zip(
            langFiles1, langFiles2, langFiles3, langPaths1, langPaths2, langPaths3
        ):
            ...
            if fullPath1:
                fh.write(
                    "<td class='td_image'><img src='"
                    + fileUri
                    + fullPath1
                    + "' height='"
                    + str(args.imageheight)
                    + "' width='"
                    + str(args.imagewidth)
                    + "' loading='lazy' style='background-color:black;'/></td>\n"
                )
            if fullPath2:
                fh.write(
                    "<td class='td_image'><img src='"
