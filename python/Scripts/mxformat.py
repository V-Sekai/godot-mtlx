...
def main():
    ...
    parser.add_argument(
        "--yes", dest="yes", action="store_true", help="Proceed without asking for confirmation from the user."
    )
    parser.add_argument(
        "--upgrade",
        dest="upgrade",
        action="store_true",
        help="Upgrade documents to the latest version of the standard.",
    )
    ...
    if not opts.yes:
        if opts.upgrade:
            question = "Would you like to upgrade all %i documents to MaterialX v%i.%i in place (y/n)?" % (
                len(validDocs),
                mxVersion[0],
                mxVersion[1],
            )
