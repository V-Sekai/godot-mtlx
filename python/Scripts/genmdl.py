...
def _getSubDirectories(libraryPath):
    return [name for name in os.listdir(libraryPath) if os.path.isdir(os.path.join(libraryPath, name))]
...
def _mapType(typeName, typeMap, functionName):
    if "mx_constant_filename" == functionName:
        ...
        ("transformpoint" in functionName) or ("transformvector" in functionName) or ("transformnormal" in functionName)
    ):
        if typeName == "string":
...
def _writeOperatorFunc(file, outputType, arg1, functionName, arg2):
    if outputType == "color4":
        ...
        file.write(
            INDENT + "return " + outputType + "(" + arg1 + ") " + functionName + " " + outputType + "(" + arg2 + ");\n"
        )
    else:
...
def _writeTranformSpace(file, outputType, functionName, input, fromspace, tospace):
    ...
    file.write(
        INDENT + "return mk_" + outputType + "( state::" + functionName + "(fromSpace, toSpace, " + input + "));\n"
    )
...
def writeNormalMap(file):
    ...
    file.write(
        INDENT
        + "    return ::math::normalize(mxp_tangent * v.x * mxp_scale + B * v.y * mxp_scale + mxp_normal * v.z);\n"
    )
    file.write(INDENT + "}\n")
...
def _writeRemap(file, outputType):
    if outputType == "color4":
        ...
        file.write(
            INDENT + "color4 val3 = mx_multiply(val2, mx_subtract(mk_color4(mxp_outhigh), mk_color4(mxp_outlow)));\n"
        )
        ...
        if outputType == "material":
            ...
            if functionName in functionTypeMap:
                ...
                if nodeCategory == "constant":
                    ...
                    if outputType != "color4":
                        _writeThreeArgumentFunc(
                            file, outputType, "::math::lerp", "mxp_fg", "(mxp_bg+mxp_fg)", "mxp_mix"
                        )
                    else:
                        file.write(
                            INDENT + "color rgb = ::math::lerp(mxp_fg.rgb, mxp_bg.rgb+mxp_fg.rgb, color(mxp_mix));\n"
                        )
                    ...
                    if outputType != "color4":
                        _writeThreeArgumentFunc(
                            file, outputType, "::math::lerp", "mxp_bg", "(mxp_bg-mxp_fg)", "mxp_mix"
                        )
                    else:
                        file.write(
                            INDENT + "color rgb = ::math::lerp(mxp_bg.rgb, mxp_bg.rgb-mxp_fg.rgb, color(mxp_mix));\n"
                        )
                    ...
                    if outputType != "color4":
                        _writeThreeArgumentFunc(
                            file, outputType, "::math::lerp", "mxp_bg", "math::abs(mxp_bg-mxp_fg)", "mxp_mix"
                        )
                    else:
                        file.write(
                            INDENT
                            + "color rgb = ::math::lerp(mxp_bg.rgb, math::abs(mxp_bg.rgb-mxp_fg.rgb), color(mxp_mix));\n"
                        )
                        file.write(
                            INDENT + "float a  = ::math::lerp(mxp_bg.a, math::abs(mxp_bg.a-mxp_fg.a), mxp_mix);\n"
                        )
                        file.write(INDENT + "return color4(rgb,a);\n")
                    wroteImplementation = True
                    ...
                    if outputType != "color4":
                        ...
                        file.write(
                            INDENT + "color rgb = ::math::lerp(mxp_bg.rgb," + dodgeStringRGB + ", color(mxp_mix));\n"
                        )
                    ...
                    if outputType != "color4":
                        _writeThreeArgumentFunc(
                            file, outputType, "::math::lerp", "mxp_bg", "mxp_bg+mxp_fg-mxp_bg*mxp_fg", "mxp_mix"
                        )
                    else:
                        file.write(
                            INDENT
                            + "color rgb = ::math::lerp(mxp_bg.rgb, mxp_bg.rgb+mxp_fg.rgb-mxp_bg.rgb*mxp_fg.rgb, color(mxp_mix));\n"
                        )
                        file.write(
                            INDENT
                            + "float a  = ::math::lerp(mxp_bg.a, mxp_bg.a+mxp_fg.a-mxp_bg.a*mxp_fg.a, mxp_mix);\n"
                        )
                        file.write(INDENT + "return color4(rgb,a);\n")
                    wroteImplementation = True
                    ...
                    if outputType == "float2":
                        ...
                        _writeOperatorFunc(
                            file, outputType, "mxp_fg", "*", "mx_multiply_color4FA(mxp_bg, 1.0-mxp_fg.a)"
                        )
                    wroteImplementation = True
                    ...
                    _writeTranformSpace(
                        ...
                        outputType,
                        "transform_vector",
                        "state::texture_tangent_u(mxp_index)",
                        "mx_coordinatespace_type_model",
                        "mxp_space",
                    )
                    wroteImplementation = True
                    ...
                    _writeTranformSpace(
                        ...
                        outputType,
                        "transform_vector",
                        "state::texture_tangent_v(mxp_index)",
                        "mx_coordinatespace_type_model",
                        "mxp_space",
                    )
                    wroteImplementation = True
                    ...
                    if outputType == "float2":
                        ...
                        file.write(
                            INDENT + "float4 val = mk_float4(mxp_fg) + (mk_float4(mxp_bg)*(1.0-mk_float4(mxp_fg).y));\n"
                        )
