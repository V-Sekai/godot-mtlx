...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a translated baked version of each material in the input document."
    )
    parser.add_argument(
        "--width",
        dest="width",
        type=int,
        default=0,
        help="Specify an optional width for baked textures (defaults to the maximum image height in the source document).",
    )
    parser.add_argument(
        "--height",
        dest="height",
        type=int,
        default=0,
        help="Specify an optional height for baked textures (defaults to the maximum image width in the source document).",
    )
    parser.add_argument(
        "--hdr",
        dest="hdr",
        action="store_true",
        help="Bake images with high dynamic range (e.g. in HDR or EXR format).",
    )
    parser.add_argument(
        "--path",
        dest="paths",
    ...
    parser.add_argument(
        ...
        type=mx.stringToBoolean,
        default=True,
        help="Specify whether to write baked materials to seprate MaterialX documents. Default is True",
    )
    if platform == "darwin":
        parser.add_argument(
            "--glsl",
            ...
            default=False,
            type=bool,
            help="Set to True to use GLSL backend (default = Metal).",
        )
