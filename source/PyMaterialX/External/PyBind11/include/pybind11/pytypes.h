...
class object : public handle
{
  public:
    ...
    object(object&& other) noexcept :
        handle(other) { other.m_ptr = nullptr; }
...
struct error_fetch_and_normalize
{
    ...
    explicit error_fetch_and_normalize(const char* called)
    {
        ...
        if (!m_type)
        {
            pybind11_fail("Internal error: " + std::string(called) + " called while "
                                                                     "Python error indicator not set.");
        }
        const char* exc_type_name_orig = detail::obj_class_name(m_type.ptr());
        if (exc_type_name_orig == nullptr)
        {
            pybind11_fail("Internal error: " + std::string(called) + " failed to obtain the name "
        ...
        {
            pybind11_fail("Internal error: " + std::string(called) + " failed to normalize the "
                                                                     "active exception.");
        }
