...
void GLRenderPipeline::updateAlbedoTable(int tableSize)
{
    ...
    {
        std::static_pointer_cast<mx::GlslMaterial>(material)->getProgram()->bindUniform(mx::HW::ALBEDO_TABLE_SIZE, mx::Value::createValue(tableSize));
    }
                ...
                if (imageHandler->bindImage(_viewer->_shadowMap, blurSamplingProperties))
                {
                    ...
                    if (textureLocation >= 0)
                    {
                        std::static_pointer_cast<mx::GlslMaterial>(_viewer->_shadowBlurMaterial)
                            ->getProgram()
                            ->bindUniform("image_file", mx::Value::createValue(textureLocation));
                    }
                }
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
